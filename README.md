...
- **策略验证** - 虚拟账户模拟交易，策略回测框架（DCA 定投、均线择时）
...
│   │   └── account.py               # 虚拟账户操作
│   └── backtest/
│       ├── models.py                # Signal, BacktestResult, ExecutedTrade
│       ├── engine.py                # 回测引擎（T+1 执行）
│       └── strategies/
│           ├── base.py              # Strategy 抽象接口
│           ├── dca.py               # DCA 定投策略
│           └── ma.py                # MA 均线择时策略
│
├── service/
